...
import (
	...
	shimprovider "github.com/MathiasPius/terraform-provider-zfs/shim"
...
func makeDataSource(mod string, name string) tokens.ModuleMember {
	...
	return tfbridge.MakeDataSource("zfs", mod, "get"+name)
}

func makeResource(mod string, res string) tokens.Type {
	return tfbridge.MakeResource("zfs", mod, convertName(res))
}
...
func Provider() tfbridge.ProviderInfo {
	...
	prov := tfbridge.ProviderInfo{
		...
		Resources: map[string]*tfbridge.ResourceInfo{
			"zfs_pool":    {Tok: makeResource(mainMod, "zfs_pool")},
			"zfs_dataset": {Tok: makeResource(mainMod, "zfs_dataset")},
		},
		DataSources: map[string]*tfbridge.DataSourceInfo{
			"zfs_pool":    {Tok: makeDataSource(mainMod, "zfs_pool")},
			"zfs_dataset": {Tok: makeDataSource(mainMod, "zfs_dataset")},
		},
		...
		Golang: &tfbridge.GolangInfo{
			ImportBasePath: filepath.Join(
				fmt.Sprintf("github.com/MaienM/pulumi-%[1]s/sdk/", "zfs"),
